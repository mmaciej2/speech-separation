...
def batch_mag_mse(est_mag, oracle_mag):
  error = oracle_mag - est_mag
  error = error ** 2
  n_batch, n_srcs, f, t = error.shape
  total_error = torch.sum(torch.reshape(error, (n_batch, n_srcs*f*t)), dim=-1)
...
def compute_loss(model, epoch, batch_sample, plotdir=""):
  ...
  stft = stft.unsqueeze(-1).permute(0, 3, 1, 2)  # [batch, 1, f, t]
  mix_mag = conv_stft.stft_mag(stft)
  est_mag = conv_stft.apply_mask(mix_mag, masks)
  ...
  oracle_mag = conv_stft.stft_mag(srcs_stft)

  losses = torch.stack([batch_mag_mse(est_mag, oracle_mag[:, torch.LongTensor(perm)]) for perm in itertools.permutations(range(model.num_srcs))], dim=-1)
